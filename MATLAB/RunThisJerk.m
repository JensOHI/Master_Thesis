...
T=[0 5]; %Time of waypoints ... they must be in order. 
Pos=[-0.25 0 0.25; 0.25 0 0.40]; % Positions of waypoints ... Each row represents (x y z) of a waypoint ... they must be in order.
...
%              
[t,Cj,PPj,VVj,AAj,POSj,VELj,ACCj,Pj,Vj,Aj] = MinimumJerkGenerator(T,Pos,v0,vf,a0,af, true);

forces = 5 * Aj;
figure('WindowState', 'maximized')
for i=1:1:min(size(forces))
subplot(min(size(forces)),1,i)
txt=['Force in D_',num2str(i),' (N)' ];
plot(t,forces(i,:),'DisplayName',"Forces",'LineWidth',3)
grid on
legend show;
ylabel("Forces");
xlabel('Time (sec)')
